...
import (
	...
	"github.com/suryansh74/auth_refresh/services"
...
func (ac *AuthController) VerifyEmail(c *fiber.Ctx) error {
	...
	if err := database.DB.Where("email = ? AND email_verify_token = ?", req.Email, req.Token).First(&user).Error; err != nil {
...
func (ac *AuthController) Login(c *fiber.Ctx) error {
	...
	database.DB.Model(&models.RefreshToken{}).Where("user_id = ? AND is_revoked = false AND expires_at > ?", user.ID, time.Now()).Count(&activeTokenCount)
...
func (ac *AuthController) RefreshToken(c *fiber.Ctx) error {
	...
	if err := database.DB.Preload("User").Where("token = ? AND is_revoked = false", req.RefreshToken).First(&refreshToken).Error; err != nil {
...
func (ac *AuthController) ForgotPassword(c *fiber.Ctx) error {
	...
	resetToken := ac.jwtService.GenerateRandomToken()
	resetExpiry := time.Now().Add(1 * time.Hour) // 1 hour
	...
	// Send reset email
	if err := ac.emailService.SendPasswordResetEmail(&user, resetToken); err != nil {
	...
	logger.Info("Password reset requested", zap.String("email", user.Email))
...
func (ac *AuthController) ResetPassword(c *fiber.Ctx) error {
	...
	req, err := validation.ValidateResetPassword(c)
	if err != nil {
	...
	if err := database.DB.Where("email = ? AND reset_token = ?", req.Email, req.Token).First(&user).Error; err != nil {
		logger.Warn("Invalid reset token", zap.String("email", req.Email))
		return c.Status(400).JSON(fiber.Map{
			"success": false,
	...
	if user.ResetTokenExpiry.Before(time.Now()) {
		logger.Warn("Reset token expired", zap.String("email", req.Email))
		return c.Status(400).JSON(fiber.Map{
			"success": false,
...
func (ac *AuthController) Logout(c *fiber.Ctx) error {
	...
	var refreshToken models.RefreshToken
	result := database.DB.Where("token = ? AND is_revoked = false", req.RefreshToken).First(&refreshToken)
	if result.Error != nil {
...
func (ac *AuthController) VerifyEmailGet(c *fiber.Ctx) error {
	...
	var user models.User
	if err := database.DB.Where("email = ? AND email_verify_token = ?", email, token).First(&user).Error; err != nil {
...
func (ac *AuthController) ResetPasswordGet(c *fiber.Ctx) error {
	...
	var user models.User
	if err := database.DB.Where("email = ? AND reset_token = ?", email, token).First(&user).Error; err != nil {
...
func (ac *AuthController) LogoutAll(c *fiber.Ctx) error {
	...
	// Revoke all refresh tokens for this user
	result := database.DB.Model(&models.RefreshToken{}).Where("user_id = ? AND is_revoked = false", userID).Update("is_revoked", true)
